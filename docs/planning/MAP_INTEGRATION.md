...
### 메인 뷰 (투표 생성 페이지)
- **초기 진입**: 투표 제목 입력 모달 (필수 입력 유도)
- **지도 로딩**: Hybrid Geolocation Strategy 적용
    1. **1차 시도 (High Accuracy)**: GPS/정밀 위치 (Timeout: 5s)
    2. **2차 시도 (Low Accuracy)**: 실패 시 Wi-Fi/Cell 기반 (Timeout: 5s) - 실내/모바일 대응
    3. **실패 시**: 기본 위치(속리산) + 에러 토스트(E1~E3) 표시
- 초기 지도 범위는 유저 현재 위치 (실패 시 기본 위치)
- 식당은 지도 마커로 표시
