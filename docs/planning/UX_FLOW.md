...
**화면 구성**:
- 📝 **제목 입력 모달 (진입 시 최초 노출)**: "오늘의 투표 제목은?"
...
**입력**: 
1. **제목 입력 (선행)**: 모달에서 제목 입력 후 지도 진입
2. **후보 장소 선택**: 지도 마커 클릭 → Bottom Sheet에서 추가 / 검색 모달에서 추가
3. **마감 시간 설정**: 하단 DateTimePicker
...
**특징**:
- **수동 종료**: 누구나 "🔒 지금 마감하기" 버튼으로 조기 종료 가능
